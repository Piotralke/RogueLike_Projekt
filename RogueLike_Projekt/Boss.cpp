...
void boss::Update(float deltaTime, std::vector<Bullet>& bulletVec, std::vector<monster>& monsterVec, hero player, sf::Texture* ghostTexture)
{
	...
	if (fire_delay_clock.getElapsedTime().asSeconds() >= fire_rate && shooting == true)
	{
		...
		Bullet newBullet(bullet_size, body.getPosition(), shot_speed, damage, -shottingDir, arrow, getDirDeg(shottingDir));
		Bullet newBullet1(bullet_size, { body.getPosition().x, body.getPosition().y+10}, shot_speed, damage, -shottingDir + test1, arrow, getDirDeg(shottingDir));
		Bullet newBullet2(bullet_size, { body.getPosition().x, body.getPosition().y-10}, shot_speed, damage, -shottingDir - test1, arrow, getDirDeg(shottingDir));
